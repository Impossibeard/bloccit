...
RSpec.describe QuestionsController, type: :controller do
  ...
  describe "POST create" do
    it "increases the number of questions by 1" do
      expect{post :create, question: {title: RandomData.random_question, body: RandomData.random_paragraph, resolved: true}}.to change(Question, :count).by(1)
    end
    it "assigns the new question to @question" do
      post :create, question: {title: RandomData.random_question, body: RandomData.random_paragraph, resolved: true}
  ...
  describe "PUT update" do
    it "updates question with expected attributes" do
      ...
      put :update, id: my_question.id, question: {title: new_title, body: new_body, resolved: new_check}

      updated_question = assigns(:question)
      expect(updated_question.id).to eq my_question.id
      expect(updated_question.title).to eq new_title
      expect(updated_question.body).to eq new_body
      expect(updated_question.resolved).to eq new_check
    end
