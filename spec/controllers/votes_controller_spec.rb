...
RSpec.describe VotesController, type: :controller do
  ...
  let(:my_vote) { Vote.create!(value: 1) }
   ...
   context "signed in user" do
     ...
     describe "POST up_vote" do
       it "the users first vote increates number of post votes by one" do
         votes = user_post.votes.count
         post :up_vote, format: :js, post_id: user_post.id
         expect(user_post.votes.count).to eq(votes + 1)
       end
       it "the users second vote does not increate the number of votes" do
         post :up_vote, format: :js, post_id: user_post.id
         votes = user_post.votes.count
         post :up_vote, format: :js, post_id: user_post.id
       ...
       it "returns http success on posts show page" do
         request.env["HTTP_REFERER"] = topic_post_path(my_topic, user_post)
         post :up_vote, format: :js, post_id: user_post.id
         expect(response).to have_http_status(:success)
       end
       it "returns http success on posts topic show" do
     ...
     describe "POST down_vote" do
       ...
       it "returns http success on topics show page" do
         request.env["HTTP_REFERER"] = topic_path(my_topic)
         post :down_vote, format: :js, post_id: user_post.id
         expect(response).to have_http_status(:success)
       end
